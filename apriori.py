...
import numpy
import csv
with open("Apriori/apriori_data.csv", 'r') as file:
    transactions = list(csv.reader(file))
    items = data = [[trans] for trans in transactions[0]]
    del transactions[0]
file.close()
min_support = (int(input("Enter the minimum support (in %): ")))/100
thrs_conf = (int(input("Enter the minimum confidence (in %): ")))/100
...
while True:
    ...
    for item in items:
        ...
        if support/len(transactions) >= min_support:
            itemset[(''.join(f'{i} ' for i in item)).rstrip()
    ...
    iteration += 1
    items = [''.join(f'{i} ' for i in list(i)).rstrip().split(' ')
...
for rule in items:
    ...
    for i in range(len(rule)-1):
        for elem in list(list(j) for j in itertools.combinations(rule, i+1)):
            confidence[(''.join(f'{char} ' for char in elem)).rstrip()] = (
                ''.join(f'{char} ' for char in numpy.setdiff1d(rule, elem))).rstrip()
...
# Output:
# Enter the minimum support (in %): 30
# Enter the minimum confidence (in %): 80
# The final rules are :
# Butter --> Bread
