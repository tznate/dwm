...
with open("data_naive.csv", 'r') as file:
...
for cls in classes:
    observations[cls] = {}
    observations[cls]['count'] = sum(
        [1 if sample[-1] == cls else 0 for sample in data])
...
print(
    f"The given data sample belongs to class {max(calculations.items(), key=lambda x:x[1])[0]}")
